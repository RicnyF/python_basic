...
budouci_datum = dnesni_datum + relativedelta(years=5)
print("Dnesni datum je {}".format(dnesni_datum))
def datum_velikonoci(rok):
    from dateutil.easter import easter
    velikonoce = easter(rok)
    return velikonoce

datum = datum_velikonoci(budouci_datum.year)
print(f"Velikonoční neděle za 5 let bude {datum.strftime('%d.%m.%Y')}.")
def najdi_nejblizsi_stedry_den_v_nedeli():
    aktualni_datum = datetime.now()
    budouci_datum = aktualni_datum
    while True:
        budouci_datum += relativedelta(days=1)
        if budouci_datum.day == 24 and budouci_datum.month == 12 and budouci_datum.weekday() == 6:
            return budouci_datum
nejblizsi_stedry_den = najdi_nejblizsi_stedry_den_v_nedeli()
print(f"Nejbližší Štědrý den v neděli bude {nejblizsi_stedry_den.strftime('%d.%m.%Y')}.")
